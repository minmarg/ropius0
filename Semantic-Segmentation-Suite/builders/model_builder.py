...
mypath = '/data/installed-software/Semantic-Segmentation-Suite/models' ##MM_edit
...
def build_model(model_name, net_input, num_classes, crop_width, crop_height, frontend="ResNet101", is_training=True):
	...
	if "ResNet101" == frontend and not os.path.isfile(os.path.join(mypath, "resnet_v2_101.ckpt")):
	    download_checkpoints("ResNet101")
	if "ResNet152" == frontend and not os.path.isfile(os.path.join(mypath, "resnet_v2_152.ckpt")):
	    download_checkpoints("ResNet152")
	if "MobileNetV2" == frontend and not os.path.isfile(os.path.join(mypath, "mobilenet_v2.ckpt.data-00000-of-00001")):
	    download_checkpoints("MobileNetV2")
	if "InceptionV4" == frontend and not os.path.isfile(os.path.join(mypath, "inception_v4.ckpt")):
	...
	if model_name == "FC-DenseNet56" or model_name == "FC-DenseNet67" or model_name == "FC-DenseNet103":
	    ...
	    network, init_fn = build_deeplabv3_plus(net_input, preset_model = model_name, frontend=frontend, num_classes=num_classes,
                                                    is_training=is_training, pretrained_dir=mypath)
            ##}}
