...
def build_frontend(inputs, frontend, is_training=True, pretrained_dir="models"):
    if frontend == 'ResNet50':
        ...
        with slim.arg_scope(resnet_v2.resnet_arg_scope()):
            ...
            #variables_to_restore = slim.filter_variables(model_variables, exclude_patterns=['resnet_v2_101/conv1/weights'])
            variables_to_restore = slim.filter_variables(model_variables, exclude_patterns=['resnet_v2_101/conv1'])
            ##print(variables_to_restore)
            init_fn = slim.assign_from_checkpoint_fn(model_path=os.path.join(pretrained_dir, 'resnet_v2_101.ckpt'), var_list=variables_to_restore, ignore_missing_vars=True)
            ##}}
